...
if __name__ == '__main__':
    with open('./config/config_RealBlurJ_bsd_gopro_pretrain_ragan-ls.yaml', 'r') as f:
        config = yaml.load(f)

    batch_size = config.pop('batch_size')
    get_dataloader = partial(DataLoader, batch_size=batch_size, num_workers=6, shuffle=True, drop_last=True)

    datasets = map(config.pop, ('train', 'val'))
    datasets = map(PairedDataset.from_datalist, datasets)
